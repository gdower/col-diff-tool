...
class Taxon:
    ...
    def scrape_infraspecies(self, rows):
        parsed_infraspecies = []
        for row in rows:
            if clean_str(row.get_text()) != '':
                parsed_infraspecies.append(Nomen(clean_str(row.get_text())).normalized)
        return parsed_infraspecies
    ...
    def scrape_taxon(self, url):
        ...
        if soup.find('p', text='Species not found') is None:
            ...
            if soup.find('th', text='Infraspecific taxon:') is not None:
                infra_td = soup.find('th', text='Infraspecific taxon:').find_next('td')
                self.infraspecies = self.scrape_infraspecies(infra_td.find_all('a'))
